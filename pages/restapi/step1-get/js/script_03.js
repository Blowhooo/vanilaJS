...
async function fetchUsers() {
    ...
    try{
        ...
        // 5. HTML 생성하여 결과 표시
        if(users) {
            showStatus(usersStatus, `${users.length}명의 데이터를 불러왔습니다`, "success");
            ...
            showStatus(usersStatus, `JSON 파싱에 실패했습니다`, "error");
        }

        
    } catch(error) {
        // 6. 에러 발생 시 에러 메시지 표시
        showStatus(usersStatus, `에러코드 : ${error.message}`, "error");
        console.error('유저 데이터 불러오기를 실패했습니다');
    }
